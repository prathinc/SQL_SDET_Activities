...
select customer_id , max(purchase_amount) as maximun_amount from orders group by customer_id;
-- Write an SQL statement to find the highest purchase amount on '2012-08-17' for each salesman with their ID.
select salesman_id, order_date, max(purchase_amount) as "highest amount" from orders where order_date = '2012-08-17' group by salesman_id, order_date;
